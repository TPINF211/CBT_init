...
public class ApprobationPropositionsPanel extends JPanel {
    ...
    public ApprobationPropositionsPanel() {
        ...
        JLabel infoLabel = new JLabel("Approuver les propositions après vote du bureau et vote des membres");
        ...
        CustomButton viewResultsBtn = new CustomButton("Voir Résultats Votes");
        CustomButton approveBtn = new CustomButton("Approuver/Rejeter");
        CustomButton refreshBtn = new CustomButton("Actualiser");
        
        viewResultsBtn.addActionListener(e -> voirResultats());
        approveBtn.addActionListener(e -> approuverProposition());
        refreshBtn.addActionListener(e -> actualiserTable());
        
        controlPanel.add(new JLabel("Status:"));
        controlPanel.add(statusCombo);
        controlPanel.add(viewResultsBtn);
        ...
        try {
            ...
            if (results != null) {
                String message = String.format(
                    ...
                    results.getVotesOui(),
                    results.getVotesNon(),
                    results.getTotalVotes(),
                    results.getPourcentageOui()
                );
                JOptionPane.showMessageDialog(this, message, "Résultats", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(this, "Aucun vote pour cette proposition");
            }
        } catch (Exception e) {
    ...
    private void approuverProposition() {
        int row = table.getSelectedRow();
        if (row < 0) {
            JOptionPane.showMessageDialog(this, "Sélectionnez une proposition!");
            return;
